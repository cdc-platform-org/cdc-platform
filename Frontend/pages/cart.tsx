...
function CartContent() {
  ...
  const switchLocale = (nextLocale: 'ka' | 'en') => {
        ...
        <div className="max-w-6xl mx-auto flex justify-between items-center">
          ...
          <div className="flex items-center gap-4">
            <div className={`flex border rounded-xl p-1 text-[11px] font-bold ${darkMode ? 'bg-slate-800 border-slate-700' : 'bg-slate-100 border-slate-200'}`}>
              <button type="button" onClick={() => switchLocale('ka')} className={`px-3 py-1 rounded-lg transition border-none cursor-pointer font-bold ${lang === 'ka' ? 'bg-slate-900 text-white shadow-sm' : 'text-slate-400 bg-transparent'}`}>KA</button>
      ...
      <div className="max-w-6xl mx-auto px-6 py-16 grid md:grid-cols-3 gap-10">
        <div className="md:col-span-2 space-y-6">
          <div className="flex items-center gap-3 mb-8">
            ...
            <h2 className={`text-2xl font-black custom-title m-0 ${darkMode ? 'text-white' : 'text-slate-900'}`}>{t('cartTitle')}</h2>
          </div>

          {cartItems.length === 0 ? (
            <div className={`rounded-2xl p-16 text-center border ${darkMode ? 'bg-[#0e1422] border-slate-800' : 'bg-white border-slate-200/60'}`}><p className="text-slate-400 font-bold text-sm">{t('cartEmpty')}</p></div>
          ) : (
            cartItems.map((item) => (
          ...
          <form onSubmit={handleCheckout} className="space-y-4">
            {paymentMethod === 'card' ? (
              <div className="space-y-4">
                ...
                <button type="submit" className="w-full bg-slate-900 text-white font-bold py-4 rounded-xl text-xs uppercase border-none cursor-pointer active:scale-95 duration-150">{isPaying ? t('paying') : t('payCard', { total: totalPrice, symbol: currencySymbols[currency] })}</button>
              </div>
            ) : (
              <div className="space-y-4">
                ...
                <button type="submit" className="w-full bg-[#003087] text-white font-bold py-4 rounded-xl text-xs uppercase border-none cursor-pointer active:scale-95 duration-150 shadow-md">{isPaying ? t('redirect') : t('payPaypal', { total: totalPrice, symbol: currencySymbols[currency] })}</button>
              </div>
            )}
          </form>
        </div>
