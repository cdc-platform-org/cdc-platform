...
export type ApiErrorReason = 'PENDING_APPROVAL' | 'BANNED' | 'DEACTIVATED' | 'ROLE_RESTRICTED' | 'DAILY_LIMIT' | 'VERIFICATION_REQUIRED';
...
const RAW_MESSAGE_MATCHERS: { test: (message: string) => boolean; reason: ApiErrorReason }[] = [
  ...
  { test: (m) => m.includes('daily posting limit'), reason: 'DAILY_LIMIT' },
  { test: (m) => m.includes('Account verification is required'), reason: 'VERIFICATION_REQUIRED' },
