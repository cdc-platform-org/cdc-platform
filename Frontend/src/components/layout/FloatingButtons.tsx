...
// Previously hardcoded Georgian-only with no locale branching at all — every
// non-ka visitor (including en) saw Georgian button labels on the homepage.
const LABELS: Record<SupportedLocale, { assistant: string; whatsapp: string }> = {
  ka: { assistant: 'მიწერე ასისტენტს', whatsapp: 'მოგვწერე WhatsApp-ში' },
  en: { assistant: 'Chat with assistant', whatsapp: 'Message us on WhatsApp' },
  de: { assistant: 'Mit Assistent chatten', whatsapp: 'Schreiben Sie uns auf WhatsApp' },
  es: { assistant: 'Chatear con el asistente', whatsapp: 'Escríbenos por WhatsApp' },
  fr: { assistant: "Discuter avec l'assistant", whatsapp: 'Écrivez-nous sur WhatsApp' },
  uk: { assistant: 'Написати асистенту', whatsapp: 'Напишіть нам у WhatsApp' },
};
...
export default function FloatingButtons({ onAssistantClick }: FloatingButtonsProps) {
  ...
  const t = LABELS[resolveLocale(router.locale)];
  ...
  // max-w raised from 220px and `truncate` added (adds text-ellipsis) — the
  // Georgian/English labels fit at 220px, but this pill is now localized
  // into 6 languages and a German/French translation of either label runs
  // meaningfully longer; without an ellipsis fallback a translation that
  // still exceeds the cap gets silently cut off mid-word instead of
  // trailing off visibly.
  const labelClass = (expanded: boolean) =>
    `truncate text-sm font-bold transition-all duration-500 ease-in-out ${
      expanded ? 'max-w-[280px] opacity-100 pr-5' : 'max-w-0 opacity-0 pr-0'
    }`;
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col gap-3 items-end">
      ...
      >
        ...
        <span className={labelClass(isExpanded('assistant'))}>{t.assistant}</span>
      </button>
      ...
      >
        ...
        <span className={labelClass(isExpanded('whatsapp'))}>{t.whatsapp}</span>
      </a>
    </div>
  );
}
