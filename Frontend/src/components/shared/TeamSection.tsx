          ...
          {members.map((member) => {
            ...
            return (
              ...
              >
                ...
                <h3 className="font-black text-lg mb-1">{localized.name}</h3>
                <span className="text-[11px] text-cyan-500 font-bold block mb-4 uppercase tracking-wider">{localized.role}</span>
                {localized.bio && <p className="text-sm text-slate-400 leading-relaxed font-medium">{localized.bio}</p>}
              </div>
            );
          })}
        </div>
